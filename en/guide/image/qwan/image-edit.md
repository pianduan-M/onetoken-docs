...
| └─ └─ └─ **image** | `string` | Yes | - Formats: JPG, JPEG, PNG, BMP, TIFF, WEBP, GIF.<br />- Output format is PNG; for GIF input, only the first frame is processed.<br />- Recommended resolution: both width and height between 384 and 3072 pixels.<br />- File size: <=10MB.<br />- For file uploads, the platform auto-converts to base64. For public URLs, use Qwen-format parameters.<br />**Supported input forms**:<br />1) Public URL (HTTP/HTTPS), e.g. `https://help-static-aliyun-doc.aliyuncs.com/file-manage-files/zh-CN/20250925/fpakfo/image36.webp`<br />2) Temporary OSS URL (obtained via upload), e.g. `oss://dashscope-instant/xxx/2024-07-18/xxx/cat.png`<br />3) Base64 string, e.g. `data:image/jpeg;base64,GDU7MtCZz...` |
